...
# with seperate vFalse
# External parameters types: v, B, A, t0, vS, BS, AS, es, tf, gf, ts, N
...
#   evidence accumulation model being used is present.
{
  ...
  par.df$v[par.df[,"v"]==Inf] <- par.df[1,"vFalse"]
  ...
  par.df[,c("v","B","A","vS","BS","AS","t0","t0sg","tf","gf","ts","N")]
}

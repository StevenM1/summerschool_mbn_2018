...
#   External parameters types: A,A1,A2, B, D1, D2, t0, meanlog_v, sdlog_v, st0 = 0 (optional)
#   Internal parameters types: A,A1,A2, b, d1, d2, t0, meanlog_v, sdlog_v, st0 = 0 (optional)
...
transform.dmc <- function(par.df) 
{
  ...
  par.df$b <- par.df$d2+par.df$B+par.df$A
  
  
  par.df[,c("A","A1","A2", "b","d1","d2","p_flc", "t0","meanlog_v","sdlog_v","st0")]
}

random.dmc<- function(n,p.df,model)
{
  rlba.lnorm.3C_A(n,A=p.df$A,A1=p.df$A1,A2=p.df$A2,d1=p.df$d1,d2=p.df$d2,b=p.df$b,t0=p.df$t0[1], 
                   p_flc=p.df$p_flc,meanlog_v=p.df$meanlog_v,sdlog_v=p.df$sdlog_v,st0=p.df$st0[1])
}


likelihood.dmc <- function(p.vector,data,ok.types=c("lnorm3C"),min.like=1e-10) 
  ...
  for ( i in row.names(attr(data,"model")) ) if ( !attr(data,"cell.empty")[i] )
  {
    ...
    likelihood[ attr(data,"cell.index")[[i]] ] <-
      switch(attr(attributes(data)$model,"type"),
             lnorm3C=n1PDF.lnorm.3C_A(t=data$RT[attr(data,"cell.index")[[i]]],

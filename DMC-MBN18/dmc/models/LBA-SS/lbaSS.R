# Stop-signal context independent parameterization n-choice LBA, 
# External parameters types: v, sv, B, A, t0, vS, svS, BS, AS, t0sg, tf, gf, ts
# Internal parameters types: v, sv, B, A, t0, vS, svS, BS, AS, t0sg, tf, gf, ts
#
...
#   evidence accumulation model being used is present.
{
  ...
  par.df["NR",c("v","sv","B","A")] <- par.df["NR",c("vS","svS","BS","AS")]
  par.df[,c("v","sv","B","A","vS","svS","BS","AS","t0","t0sg","tf","gf","ts")]
}

random.dmc<- function(n,p.df,model,SSD=Inf,staircase=NULL,TRIALS)
{
  rLBAss(n=n,
         v=p.df$v,sv=p.df$sv,B=p.df$B,A=p.df$A,
         t0=p.df$t0[1],t0sg=p.df$t0sg[1],
         tf=p.df$tf[1],gf=p.df$gf[1],
         ts=p.df$ts[1],TRIALS=TRIALS,
...
# Returns vector of likelihoods for each RT in data (in same order)
{
  ...
  for ( i in row.names(attr(data,"model")) ) if ( !attr(data,"cell.empty")[i] )
  {
    ...
    likelihood[ attr(data,"cell.index")[[i]] ] <-
      n1PDF.LBAss(rt=data$RT[attr(data,"cell.index")[[i]]],
          v=p.df$v,
          B=p.df$B,
          sv=p.df$sv,
          A=p.df$A,

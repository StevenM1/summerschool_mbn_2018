...
# External parameters types: mean, sd, t0, meanS, sdS, t0sg, tf, gf, ts
...
#      r = go response production time ycan only identify two functions of these:
...
#
# NB3: ts is slope of TRIALS covariate on meanlog
...
#   evidence accumulation model being used is present.
{
  ...
  par.df["NR",c("mean","sd")] <- par.df["NR",c("meanS","sdS")]
  S=log(1+(par.df$sd^2)/exp(2*log(par.df$mean)))
  par.df$meanlog <- -S/2+log(par.df$mean) 
  par.df$sdlog <- sqrt(S)

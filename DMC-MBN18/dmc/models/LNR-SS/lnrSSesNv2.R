...
# External parameters types: meanlog, sdlog, t0, meanlogS, sdlogS, es, tf, gf, ts, N
...
#   evidence accumulation model being used is present.
{
  ...
  par.df$meanlog[par.df[,"meanlog"]==Inf] <- par.df[1,"meanlogFalse"]
  par.df$sdlog[par.df[,"sdlog"]==Inf] <- par.df[1,"sdlogFalse"]
  par.df["NR",c("meanlog","sdlog")] <- par.df["NR",c("meanlogS","sdlogS")]
  par.df$t0sg <- par.df$es
  par.df[,c("meanlog","sdlog","meanlogS","sdlogS","t0","t0sg","tf","gf","ts","N")]
}

random.dmc<- function(n,p.df,model,SSD=Inf,staircase=NULL,TRIALS=NULL)
{
  rlnrss(n,meanlog=p.df$meanlog[1:p.df$N[1]],sdlog=p.df$sdlog[1:p.df$N[1]],
  ...
  for ( i in row.names(attr(data,"model")) ) if ( !attr(data,"cell.empty")[i] )
  {
    ...
    likelihood[ attr(data,"cell.index")[[i]] ] <-
      n1PDF.lnrss(rt=data$RT[attr(data,"cell.index")[[i]]],
          meanlog=p.df$meanlog[1:p.df$N[1]],
          sdlog=p.df$sdlog[1:p.df$N[1]],
          t0=p.df$t0[1], 
          t0sg=p.df$t0sg[1],
          tf=p.df$tf[1],
          gf=p.df$gf[1], 

...
# External parameters types: meanlog, sdlog, t0, meanlogS, sdlogS, t0sg, tf, gf, ts
# Internal parameters types: meanlog, sdlog, t0, t0sg, tf, gf, ts
#
# NB1: Unlike lnr st0 is not available
# NB2: ts is slope of TRIALS covariate on meanlog
# 
...
#   evidence accumulation model being used is present.
{
  ...
  par.df["NR",c("meanlog","sdlog")] <- par.df["NR",c("mu","sigma")]
  par.df[,c("meanlog","sdlog","tau","t0","tf","gf","ts")]
}

random.dmc<- function(n,p.df,model,SSD=Inf,staircase=NULL,TRIALS=NULL)
{
  rlnrexgss(n,meanlog=p.df$meanlog,sdlog=p.df$sdlog,tau=p.df$tau,
         t0=p.df$t0[1],tf=p.df$tf[1],gf=p.df$gf[1],
...
# Returns vector of likelihoods for each RT in data (in same order)
{
  ...
  for ( i in row.names(attr(data,"model")) ) if ( !attr(data,"cell.empty")[i] )
  {
    ...
    likelihood[ attr(data,"cell.index")[[i]] ] <-
      n1PDF.lnrexgss(rt=data$RT[attr(data,"cell.index")[[i]]],

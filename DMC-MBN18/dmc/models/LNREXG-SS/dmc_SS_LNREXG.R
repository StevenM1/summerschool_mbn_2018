### An LNR Stop-signal EXG example with TRIGGER and GO FAILURE
# Context Independent Parameterization, i.e., seperate mu (in meanlog), sigma
# (in sdlog) and tau for stop. ASSUMES NON-BALLISTIC (can stop up to the moment
# a resonse is made).
...
load_model ("LNREXG-SS","lnrexgSS.R") 
...
model <- model.dmc(type="lnrss",               # LNR stop signal model
  ...
  p.map=list(meanlog="M",sdlog="M",            # GO accumulator's parameters 
             ...
             t0="1",tau="1",                   # GO non-decision, stop tau
...
p.vector  <- c(meanlog.true=0,meanlog.false=1,mu=.3,
               sdlog.true=0.5,sdlog.false=1,sigma=0.25,tau-.05,
               t0=.2,gf=.1,tf=.1) 

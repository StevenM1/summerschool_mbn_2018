...
{
  rlba.lnorm.3TC_A(n,A=p.df$A,A1=p.df$A1,A2=p.df$A2,d1=p.df$d1,d2=p.df$d2,b=p.df$b,t0=p.df$t0[1], 
  ...
  for ( i in row.names(attr(data,"model")) ) if ( !attr(data,"cell.empty")[i] )
  {
    ...
    likelihood[ attr(data,"cell.index")[[i]] ] <-
      switch(attr(attributes(data)$model,"type"),
             lnorm3TC=n1PDF.lnorm.3TC_A(t=data$RT[attr(data,"cell.index")[[i]]],

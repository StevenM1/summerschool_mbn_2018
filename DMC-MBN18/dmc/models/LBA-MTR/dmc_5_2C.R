...
source ("dmc/dmc.R")
load_model ("LBA-MTR","lba_2C.R")

# Note that responses2 must have values 1 (low confidence) and 2 (high confidence).
model <- model.dmc(p.map=list(A="1",B="SA",D="SA",p_flc="SA",mean_v=c("D","M"),sd_v="1",t0="SA",st0="1"),
  ...
  responses=c("LEFT","RIGHT"),responses2=c("1","2"),type="norm2C")
...
p.vector  <- c(A=.5,D.accuracy=.5,D.speed=.25,B.accuracy=.5,B.speed=.25,
               p_flc.speed=.2,p_flc.accuracy=.02,
...
profile.dmc("D.speed",    .1,  1,p.vector,data.model,ylim=NA)
profile.dmc("p_flc.speed",    0,  1,p.vector,data.model,ylim=NA)
profile.dmc("p_flc.accuracy",    0,  1,p.vector,data.model,ylim=NA)
...
is.0 <- data$R=="LEFT" & data$R2==2
is.3 <- data$R=="RIGHT" & data$R2==2
data$R <- as.numeric(data$R)
data$R[is.0] <- 0
data$R[is.3] <- 3
data$R <- factor(data$R,labels=c("LEFThi","LEFTlo","RIGHTlo","RIGHThi"))


tapply(data$RT,data$R,quantile,probs=c(.1,.3,.5,.7,.9))

dat<-matrix(unlist(tapply(data$RT,data$R,quantile,probs=c(.1,.3,.5,.7,.9))),ncol=4)

rownames(dat)<-c(.1,.3,.5,.7,.9)
colnames(dat)<-c("LEFThi", "LEFTlo","RIGHTlo", "RIGHThi")

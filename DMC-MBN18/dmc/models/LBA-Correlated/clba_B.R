# Template setup for n-choice correlated rate LBA, B=b-A parameterization
#   External parameters types: A, B, t0, mean_v, sd_v, corr_v, st0 = 0 (optional)
#   Internal parameters types: A, b, t0, mean_v, sd_v, corr_v, st0 = 0 (optional)
...
transform.dmc <- function(par.df) 
{
  ...
  par.df[,c("A","b","t0","mean_v","sd_v","corr_v", "st0")]
}


random.dmc <- function(n,p.df)
{
  rlba.cnorm(n,A=p.df$A,b=p.df$b,t0=p.df$t0, 
            mean_v=p.df$mean_v,sd_v=p.df$sd_v,corr_v=p.df$corr_v,
            st0=p.df$st0[1])
}


likelihood.dmc <- function(p.vector,data,ok.types=c("cnorm"),min.like=1e-10) 
